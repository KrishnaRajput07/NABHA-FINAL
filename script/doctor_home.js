    ...
    function openDrawer(id){
      ...
      document.getElementById('kpiPulse').textContent = p.vitals.pulse + ' bpm';
      document.getElementById('kpiSpo2').textContent = p.vitals.spo2 + ' %';
      document.getElementById('kpiTemp').textContent = p.vitals.temp;
      document.getElementById('ccText').textContent = p.cc;
      document.getElementById('hxList').innerHTML = p.hx.map(h=>`<li>${h}</li>`).join('');
      document.getElementById('medList').innerHTML = p.meds.map(m=>`<li>${m}</li>`).join('');
